...
RSpec.describe Card, type: :model do
  ...
  context 'scopes' do
    ...
    let(:card) { create(:random_c, list: list) }
    let(:card2) { create(:random_c, list: list) }
    let(:card3) { create(:random_c, list: list) }
    let(:labels) { build_list(:label, 3, dashboard: dashboard) }
    let(:label) { create(:label, dashboard: dashboard) }
    let(:label2) { create(:label, color: 'green', dashboard: dashboard) }
    ...
    #   card3.users << user
    #   users.each(&:save)
    #   expect(Card.by_assigned_users(user.id)).to contain_exactly(card2, card3)
    # end
    # it 'scope  by_assigned_users' do
    #   card.users = users
    #   card2.users = users
